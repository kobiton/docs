...
image:session-analytics:custom-search-queries.png[width=1000,alt="The custom queries parameters"]
...
After a query is built, select *Add Saved Search*, then *Save*. Now you can run this query any time you xref:session-analytics:search-for-a-session.adoc#_saved_searches[search for a session].

image:session-analytics:add-saved-search-context.png[width=1000,alt="Click Star to add Saved Search"]
...
Returns all sessions matching this status. See xref:session-analytics:search-for-a-session.adoc#_session_tags[list of session tags].
