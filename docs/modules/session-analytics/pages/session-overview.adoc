...
In the xref:about-the-session-explorer.adoc[Session Explorer], leverage the NOVA AI engine to automatically flag accessibility issues, reinspect elements from the session, validate UI using Figma, and more.
...
The full session recording. To view each test action, accessibility issue, and get links to specific test steps, use the session recording in the xref:session-explorer/session-explorer-timeline.adoc[Session Explorer] instead.
