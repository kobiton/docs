...
Learn how to enable Jira so everyone in your organization can create Jira tickets directly from their xref:session-explorer:manage-a-session.adoc[session overview].
...
Enter your Jira Host URL, email, and the xref:_create_jira_api_token[API Token] you created earlier, then select *Connect* to finish. Now users in your organization can create Jira tickets directly from a xref:session-explorer:manage-a-session.adoc[session overview].
