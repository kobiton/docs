...
After you add our library, rebuild your `.ipa` and upload it to the xref:apps:manage-your-library.adoc[app repository].
...
Save the file.If your app requires support for iOS 14 or earlier, you’ll also need to xref:_add_new_info_plist_parameters[add a few `info.plist` parameters]. Otherwise, you can vxref:_verify_the_updates[verify your changes].
