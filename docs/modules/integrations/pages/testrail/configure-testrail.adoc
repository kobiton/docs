= Add TestRail
:navtitle: Add TestRail

Learn how to add TestRail to your test suite so you can automatically update TestRail projects and display project IDs in your session overview.

image:$NEEDSIMAGE$[width=, alt=""]

== Before you start

You'll need to xref:testrail/create-a-project.adoc#_generate_an_api_key[generate a TestRail API key] and xref:testrail/create-a-project.adoc#_create_a_project[create a new TestRail project] before you start. Optionally, xref:profile:add-an-api-key.adoc[add a new Kobiton API key] if you don't want to use your default.

== Create constant variables

First, download your xref:automation-testing:auto-generate-an-appium-script.adoc[auto-generated Appium script] and open the config file in the `test` directory. The exact location and name of the config file will depend on your testing framework.
...
In the config file, assign your Kobiton xref:profile:manage-your-profile.adoc[username] and xref:profile:add-an-api-key.adoc[API key] to constant variables. Here's an example:
