...
Enter your Figma file URL and the xref:_create_figma_api_token[API key] you created earlier, then select *Connect* to finish. Now you can use the xref:session-explorer:validations/validate-ui-design.adoc[session explorer] to flag inconsistencies between your app's Figma wireframe and final design.
