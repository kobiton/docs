...
If you see the error below on iOS 17.4 and later after rebooting devices or restarting services, upgrade to deviceConnect 4.19.3 to resolve the DDI issue.
[source]
...
*** Requirements: Device must be paired with a Bluetooth bridge server, and the audio configuration must be present in `dc.ini`.
...
* Download options for *Injected Audio* and *Output Audio* tracks will appear in Session Overview.
...
** Fix behavior when selecting validation in Test Run Matrix.
