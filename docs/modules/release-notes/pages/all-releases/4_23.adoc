...
* A Raspberry Pi host configured to support remote device unlock
* An iOS device paired with the host

The same host hardware used for other Bluetooth-based features can be reused, and no physical access to the device is required during testing sessions.

Contact Kobiton support for setup instructions.

=== Important notes & limitations

Remote device unlock using a Bluetooth keyboard applies only to the following scenarios:

* iOS 26 devices in organizations with Kobiton’s managed passcode enabled (mode 1 or mode 2)
* iOS devices in organizations without passcode mode enabled (mode 0), where the passcode is set manually and updated manually in the Portal database

Remote device unlock using a Bluetooth keyboard is not supported after a device restart. In this case, the device must be unlocked physically.
...
* Keyboard mode for VoiceOver and remote passcode unlock for iOS devices
