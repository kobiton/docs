...
Refer to this xref:manual-testing:screen-reader/voice-over-manual-session.adoc[user guide,window=read-later] for iOS VoiceOver support.
...
** *Important*: The request parameters `tags` and `tag_type` for the https://api.kobiton.com/v2/docs#tag/NativeFrameworkAPI/operation/NativeFrameworkAPI_InitiateNativeSession[Initiate an Appium Turbo Test session,window=read-later] endpoint are deprecated and replaced with `device_tags` and `device_tag_type` accordingly.
