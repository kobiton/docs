...
You can now view and filter device logs by *Info* level for iOS devices (Android devices already support this type of logging).More detailed documentation for this feature is available in xref:manual-testing:device-information/device-logs.adoc#_filters[here].
