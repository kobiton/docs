...
[#_device_log_show_info_logging_for_ios_devices]
== Device Log: Show info logging for iOS devices

You can now view and filter device logs by *Info* level for iOS devices (Android devices already support this type of logging).More detailed documentation for this feature is available in xref:manual-testing:device-information/device-logs.adoc#_filters[this link].
