...
* Test Management: Preserved test management data from being purged by the scheduled cleanup script.

* Removed the Register Now (register a new organization) button as it caused issues with new Standalone users where only 1 organization is needed.

* Added the Register with SSO link to support first-time login for SSO new users.

* Optimized GPU memory handling for AI-Service.
