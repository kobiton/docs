...
To ensure Cloud and Hybrid customers can always enjoy the latest features, we added a new policy to force devices offline on the Kobiton portal if deviceConnect on the host machine is 2 major versions behind the latest. This means for the 4.14 release, all devices hosted by deviceConnect 4.12 or below will become offline. Devices hosted by deviceConnect 4.13 are still online until 4.15 is released (if not updated).
