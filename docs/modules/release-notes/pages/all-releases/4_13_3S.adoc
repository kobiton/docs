...
With iOS 17.4, Apple introduced the CoreDevice communication protocol that significantly enhances connections between iOS devices and the Kobiton platform.We implemented this innovative protocol with this release. CoreDevice eliminates the need for the special Lightning Mode networking changes introducedwith iOS 17 for iOS 17.4+ devices, simplifying your networking.

Apple's CoreDevice cannot be used with iOS 17.0-17.3 devices. These devices must be hosted on a separate Mac mini in Standard mode.
