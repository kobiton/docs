...
Kobiton does not include source branch or commit information as app metadata. To distinguish uploaded builds by branch or commit, see xref:apps:upload-an-app/using-the-kobiton-api.adoc#_identify_builds_by_branch_or_commit[Identify builds by branch or commit].
