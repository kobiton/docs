...
* `+${app_id}+`: (optional) if another version of this app is already uploaded, provide the `app_id` of the existing app. _Example_: `100`.

[NOTE]
====
You can look up `app_id` in xref:apps:app-metadata.adoc#_app_id[app metadata].
====

* `+${app_file_name}+`: (required) provide the name of the app file to upload. _Example_: `MyApp.ipa`.

Throughout this page, in each section you will be provided with an example. The example provides the commands to execute the uploading of a fake app named MyApp.ipa. We will guide you through uploading this app in each section. In the example, MyApp.ipa is a new application and you will not provide the optional app_id.

For this first one, your command would be:
...
Using the Example from the previous section, your command would be:
...
* `state`: the parsing state of the app.
** `OK`: The uploaded application is processed by the server, and is ready to use.
** `PARSING`: The uploaded application is still being processed by the server.
** `FAILURE_PARSING`: The uploaded application cannot be processed by the server and cannot be used, such as invalid file type.
...
[NOTE]
If you receive `error 404` with the message `App version ID ... doesn't exist` although you have created the application version yourself, it could be another user has made the application private or not available to your account. Contact an admin to ask for access to this application.