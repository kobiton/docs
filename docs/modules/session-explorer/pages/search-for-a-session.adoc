...
|Gaming
|Sessions using the xref:automation-testing:turbo-test-execution/run-game-driver-test.adoc[GameDriver,window=read-later] framework.

|Mixed
|Sessions combining manual and automation testing during a single session.

|CLI
|Sessions using xref:kobiton-cli:index.adoc[Kobiton Command Line Interface].
|====
