...
[IMPORTANT]
====
Follow xref:manual-testing:inject-audio.adoc[this guide,window=read-later] for more details on how to use audio injection.
====

After executing an automation session (https://github.com/kobiton/samples/tree/master/audio[sample code,window=read-later]) or a xref:manual-testing:inject-audio.adoc[manual session,window=read-later] with bluetooth audio enabled, you can download the audio tracks and metadata under the *Download Audio Tracks* section:
