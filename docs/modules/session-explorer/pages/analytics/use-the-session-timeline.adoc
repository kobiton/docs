...
* A: Accessibility
* C: Crash
* R: Response time (for _Network Payload Capture_ session)
* S: Appium self-healing (for automation session with _flexCorrect_ enabled)

image:session-explorer-accessibility-validation.png[width=250,alt="Yellow dots on the timeline showing Accessibility Validation"]
...
* xref:session-explorer:validations/validate-accessibility.adoc[Accessibility validation]
* xref:session-explorer:analytics/review-crash-logs.adoc#_review_the_crash_logs[Crash log]
* xref:session-explorer:analytics/review-network-payload-data.adoc#_review_response_times[Response time]
* xref:automation-testing:capabilities/add-flexcorrect.adoc[flexCorrect (Appium self-healing)]
