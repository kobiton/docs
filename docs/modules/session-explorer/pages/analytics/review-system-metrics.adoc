...
To view device memory usage during the test session, select the dropdown and choose *Memory*.

image:session-explorer:memory-closeup.png[width=500,alt="A closeup to Memory section of Review System Metrics"]

== View CPU usage

To view device CPU usage during the test session, select the dropdown and choose *CPU*.

image:session-explorer:cpu-closeup.png[width=500,alt="A closeup to CPU section of Review System Metrics"]

== View network usage

To view the number of bytes the device sent and received over the network during the test session, select the dropdown and choose *Network*.

image:session-explorer:network-closeup.png[width=500,alt="A closeup to Network section of Review System Metrics"]

== View battery usage

To view the device battery drain and usage during the test session, select the dropdown and choose *Battery Drain*.
