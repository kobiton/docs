...
[NOTE]
====
Accessibility checks run automatically on manual sessions. For Appium automation sessions, add `kobiton:accessibilityValidation=true` to your capabilities to see results here. See xref:automation-testing:capabilities/available-capabilities.adoc#_accessibilityValidation[`kobiton:accessibilityValidation`] for details.
====
