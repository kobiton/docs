...
[NOTE]
====
Accessibility validation is available on paid Kobiton subscriptions. Accessibility checks run automatically on manual sessions. For Appium automation sessions, add `kobiton:accessibilityValidation=true` to your capabilities so accessibility results appear on the session timeline in Session Explorer. See xref:automation-testing:capabilities/available-capabilities.adoc#_accessibilityValidation[`kobiton:accessibilityValidation`] for details.
====
