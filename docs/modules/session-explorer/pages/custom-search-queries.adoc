...
Returns all Scriptless sessions.For manual or automation sessions, see xref:_type[].
...
Returns all sessions matching this status.For more information, see xref:session-explorer:search-for-a-session.adoc#_session_tags[Session tags].
...
Returns all sessions with this session type: `manual` or `automation`. For scriptless sessions, see xref:_isscriptless[].
