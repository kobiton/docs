...
The Session Explorer automatically flags xref:_issue_types[accessibility issues] for each test session. Learn more about a specific issue by checking the xref:session-explorer:session-explorer-tools/fix-accessability-issues.adoc[issue details].
