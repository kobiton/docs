...
After creating the file, continue with either OAuth or API key authentication. If your Standalone (on-premises) Portal uses a self-signed SSL certificate, trust the certificate first.

=== Trust self-signed SSL certificate (Standalone only)

Import the certificate on each computer that runs Claude Code.
