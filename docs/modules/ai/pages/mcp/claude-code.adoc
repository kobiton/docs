...
The Kobiton MCP server URL determines whether you need a custom `.mcp.json` file in your project folder:

[cols="1,2,2",options="header"]
|===
|Environment |MCP server URL |`.mcp.json` required?

|Kobiton Cloud (default)
|`\https://api.kobiton.com/mcp`
|No. The plugin uses this URL automatically.

|Custom environment (Standalone, Cloud with custom domain, or any other deployment)
|Your custom URL, for example `\https://api-custom-domain.kobiton.com/mcp`
|Yes. You must create a `.mcp.json` file in your project folder with the custom URL.
|===
...
If your Kobiton MCP server is hosted at a URL other than `\https://api.kobiton.com/mcp`, you first need to retrieve your custom MCP server URL. See <<retrieve-mcp-url,Retrieve your MCP server URL>> for details.

Once you have your custom URL, create a `.mcp.json` file in your project folder:
...
// TODO: Add instructions for retrieving the MCP server URL. Link to related doc here.

Add `/mcp` at the end of the retrieved API base URL. This will be the MCP server URL. See below for an example:
...
Update `.mcp.json` to include the `Authorization` header. For API key authentication, the `.mcp.json` file is required regardless of which environment you use. For the default Kobiton Cloud URL, use:
...
For a custom environment, replace the `url` value with your custom MCP server URL.
