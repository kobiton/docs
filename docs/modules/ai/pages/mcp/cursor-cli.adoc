...
. Generate an API key in *Kobiton Portal > Settings > API Keys*.

. Export the credentials in the shell that launches Cursor CLI:
+
[source,bash]
----
...
----
+
Add this line to `~/.zshrc`, `~/.bashrc`, or `~/.bash_profile` to persist it across sessions.

. Configure the `kobiton` MCP server to send the credentials as an `Authorization` header:
+
[source,json]
----
{
...
Cursor expands `${env:NAME}` at startup. This syntax is specific to Cursor. The bare `${NAME}` form used by other MCP clients is sent as a literal string, and authentication fails.

The plugin ships this configuration as `.cursor/mcp.apikey-example.json`. Copy that file rather than writing the JSON by hand.

The plugin reads its MCP configuration from `.cursor/mcp.json`, declared as `mcpServers` in `.cursor-plugin/plugin.json`. Edit that file to switch the installed plugin to API key authentication. It sits in the plugin cache and is pinned to a plugin version, so reinstalling or updating the plugin replaces it and the change must be reapplied. To keep the configuration across plugin updates, register the server in a project-level `.cursor/mcp.json` instead.
