...
----
+
Replace `<username>` with your Kobiton username and `<api-key>` with the key generated in step 1. Add this line to `~/.zshrc`, `~/.bashrc`, or `~/.bash_profile` to persist it across sessions.

. Add the `Authorization` header to a project-level `.cursor/mcp.json`. The plugin ships a ready-made template at `.cursor/mcp.apikey-example.json`. Copy that file rather than writing the JSON by hand.
+
[source,json]
----
{
...
. Restart Cursor CLI. If you added the export to a shell profile, reload your shell first.
...
A project-level `.cursor/mcp.json` keeps the configuration across plugin updates. The plugin also carries its own `.cursor/mcp.json`, but that copy sits in the plugin cache pinned to a plugin version, so reinstalling or updating the plugin replaces it and you must reapply the change.
