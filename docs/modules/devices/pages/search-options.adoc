...
These are the available options when xref:search-for-a-device.adoc[searching for a device]. Once you've found a device, you can xref:install-an-app.adoc[install apps] , xref:automation-testing:auto-generate-desired-capabilities.adoc[auto-generate desired capabilities], and change xref:manage-devices.adoc[device settings].
