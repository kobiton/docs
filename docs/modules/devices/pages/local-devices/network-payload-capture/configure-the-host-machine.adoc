...
If the 2 files do not exist, follow [this guide] to create and/or export the supervision identity files and transfer them to the Mac mini host.

Edit the `dc.ini` file from the location `/usr/local/deviceconnect/` using any text editor (Create a backup of the `dc.ini` file prior to making updates).

Find the line `IOSDeviceController.WifiCaptureProxyAutomation=ui` in the content of the file.

If there is no such line, move on to set up the devices.
...
If there is no such line, add the following line to the end of the file `IOSDeviceController.WifiCaptureProxyAutomation=ui`.
