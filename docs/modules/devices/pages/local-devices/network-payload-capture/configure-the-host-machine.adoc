...
Based on the decided xref:decide_ios_option[option], prepare the device using either option 1 or 2.
...
* If there is such a line, change it to the following: `IOSDeviceController.WifiCaptureProxyAutomation=profile`. xref:device-lab-management:deviceConnect/restart-deviceconnect-services.adoc[Restart deviceConnect services,window=read-later] to apply the changes.
