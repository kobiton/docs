...
=== If the supervision identity does not exist
...
Choose a location to save the file and choose *Save*. Keep the exported `.organization` file in a safe location for later use.
...
== If the supervision identity already exists

Transfer the `.organization` file to the non-Kobiton Mac machine, then open *Apple Configurator*.

On the *Apple Configurator* menu, select *Settings*.

Select the *Organizations* tab. If there already is an organization, then move on to the next section.

If there is no organization, select the *3-dot icon* and choose *Import Organization*.

Select the `.organization` file, then choose *Import*.

Enter the password that was used to encrypt the file, then choose *Submit*.

The organization should now be imported.
