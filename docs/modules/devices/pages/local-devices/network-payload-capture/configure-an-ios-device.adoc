...
=== Install Apple Configurator on the non-Kobiton Mac machine
...
If there is no organization listed, locate the `.organization` file that was saved during the creation of the supervision identity and transfer it to the non-Kobiton Mac machine.

Open *Apple Configurator*, then select the *3-dot icon* and choose *Import Organization*.
...
Select the `.organization` file, then choose *Import*.

image:apple-configurator-locate-organization.png[width=500,alt="The Acme.organization file that is selected for import"]

Enter the password that was used to encrypt the file, then choose *Submit*.
...
Your organization should now appear in the list. Next, you will need to export this organization into 2 sets of files:

* A `.organization` file. This file does not need to be transferred to the Mac mini host, but should be kept in a safe place to import later if needed.

** Highlight the organization, then choose the *three-dot icon* and select *Export Organization*.
+
...
** Enter a strong password to encrypt the file. Note down the password in a safe place.
+
...
** Choose a name and location to save the file and choose *Save*. Keep the exported `.organization` file in a safe location for later use.
+
...
* A pair of `.crt` and `.der` files. This pair of files need to be transferred to the Mac mini host.

** Highlight the organization again, then choose the *three-dot icon* and select *Export Supervision Identity*.
+
...
** Under the selected location, there is now a pair of `.crt` and `.der` files with the same name. Rename those files to `organization.crt` and `organization.der`, in that order. Transfer these 2 files into the Mac mini host (the Mac mini with Kobiton software running).
+
[IMPORTANT]
For newer versions of Apple Configurator, the exported files will have the extension `.crt` and `.cer` instead. Rename the `.cer` file into `organization.der`.
...
* Ensure that all iOS devices in a Mac mini host are supervised by only one single supervision identity.
