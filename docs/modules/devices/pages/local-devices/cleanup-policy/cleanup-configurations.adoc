...
NOTE: Configurations that are marked with `(*)` are always active in any user-created cleanup policy or the _Thorough_ predefined policy, although they are not shown in the create or edit cleanup policy dialog.
