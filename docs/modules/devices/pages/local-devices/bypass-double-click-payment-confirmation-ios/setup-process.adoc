...
Input the passcode. Tap the black space and enter the passcode using your keyboard. Due to Apple security measures, you will not see the passcode as you enter it. After inputting the whole passcode, press the backspace or delete key on your keyboard once, then input the last digit of the passcode. Tap the Done button when it is enabled. For Private devices, contact the Kobiton Support, if needed, to retrieve the passcode set on the device.

image::bypass-apple-pay-enter-passcode.PNG[]

Holding the device, double-click the physical side button on the device again to confirm.
...
Plug the device into the Cambrionix Hub. Start a Manual Session on the device.
