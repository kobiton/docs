...
* Access the Kobiton CLI from the Downloads icon in the left navigation of the Kobiton Portal. The package is available for the following platforms:
...
[source,bash]
tar -zxvf <downloaded-cli-package>

Replace `<downloaded-cli-package>` with the file name of the package you downloaded.
...
# Obtain the following from the Profile and Settings page of the Kobiton Portal
...
[source,bash]
.Example
# The following assumes that all required export variable commands are in the file variables.env
...
Right-click the downloaded `.zip` package and select *Extract All...*, then choose *Extract*. This results in a folder with the file `kobiton.exe`.
...
[NOTE]
=====
For *Command Prompt* (cmd.exe), use the following command instead to set environment variables:
[source,bash]
...
After the CLI has been set up for the appropriate platform, run the following command from the same location as the `kobiton` file to verify the variables and confirm the CLI is working:
...
These commands can be added to your shell initialization script to persist the variables across shell sessions. The shell configuration file is typically `.bashrc`, `.zshrc`, or `.bash_profile`. Consult your shell's documentation if you're unsure.
...
[source,bash]
REM Run the following to export authentication variables
setx KOBITON_PORTAL=<PORTAL_URL>
setx KOBITON_USER=<USERNAME>
setx KOBITON_API_KEY=<API_KEY>
REM Run the following to add kobiton executable path to PATH
...
[source]
# Run the following to export authentication variables
...
# Run the following to add kobiton executable path to PATH
