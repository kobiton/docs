...
To list files on a device, use <<file-list,`kobiton file list`>>. A restricted session confines `adb-shell ls` to the permitted directories, so `ls -l /sdcard` is rejected while `ls -l /sdcard/Download/` is accepted. Run `kobiton device adb-shell --help` for the permitted directories.
...
** `kobiton-store:<APP_ID>` runs the most recently uploaded version of that app, such as `kobiton-store:662537`.
** `kobiton-store:v<APP_VERSION_ID>` pins an exact version, such as `kobiton-store:v762538`. Pin the version when a run must stay reproducible, since a later upload changes what the bare App ID resolves to. `kobiton app upload` prints both the App ID and the Version ID.
