...
NOTE: The CLI automatically saves the session token to a local file for reuse in subsequent commands. This approach is convenient for single-user workflows but is not recommended for environments with concurrent or parallel executions.
