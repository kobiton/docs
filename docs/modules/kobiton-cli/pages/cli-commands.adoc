...
The CLI automatically saves the session token to a local file for reuse in subsequent commands. This approach is convenient for single-user workflows but is not recommended for environments with concurrent or parallel executions.

The locally stored session token will be overwritten when a new CLI session is created.
...
| `<IDENTITY>` | PID or application identifier of the process to signal.
|===
...
| `-n`, `--name` | `<NAME>` | The application name, bundle, or package to install.
...
| `<KEYWORD>` | Keyword to find the app. Put `<KEYWORD>` between double quotes for space or special characters.
|===
...
[NOTE]
====
Path syntax. Android paths are absolute device paths (for example, `/data/local/tmp`). iOS paths use the `@<bundle-id>:<path>` form to address an app's sandbox (for example, `@com.company.app:Documents/`).
====
...
Retrieve the device screen as a JPEG.
...
| `<RESERVATION_ID>` | Reservation ID to terminate.
|===
...
[source,shell]
----
kobiton reservation terminate 44821
----
