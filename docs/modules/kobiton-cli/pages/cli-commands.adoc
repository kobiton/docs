...
Commands are checked against a deny-by-default whitelist before they reach the device, and the interactive shell is unavailable, so every invocation must name a command to run. Private cloud and on-premises deployments with dedicated devices are not restricted.
...
Run `kobiton device adb-shell --help` for the current list of allowed commands, permitted directories, and per-command restrictions.
====
...
# Pass arguments after -- so the CLI does not read them as its own options
kobiton device adb-shell -- logcat -d -t 200
----

Both examples run on a restricted session.

To list files on a device, use <<file-list,`kobiton file list`>>. A restricted session confines `adb-shell ls` to the permitted directories, so `ls -l /sdcard` is rejected while `ls -l /sdcard/Download/` is accepted.
...
On a dedicated device, quote the whole expression instead. The pipe then runs on the device, so less output crosses the network:
...
[NOTE]
====
`<FRAMEWORK>` is not the Appium `automationName` capability. An Appium session sets `appium:automationName` to `UiAutomator2`; `test run` takes `uiautomator`.
====
