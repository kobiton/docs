...
Commands are checked against a deny-by-default whitelist before they reach the device, and the interactive shell is unavailable, so every invocation must name a command to run. Private cloud and Standalone deployments with dedicated devices are not restricted.
