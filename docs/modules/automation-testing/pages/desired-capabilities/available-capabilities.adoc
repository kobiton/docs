...
Screenshots will be captured after each test step automatically. They'll be available in the xref:session-explorer:manage-a-session.adoc[session overview] after the test session.
...
If a device passcode was xref:devices:set-device-passcodes.adoc[set by an administrator], the value `passcode` must the pre-set passcode in order to use the device during the test session. If the device doesn't have a passcode, the value of `passcode` will be used to create one. xref:automation-testing:desired-capabilities/add-passcode.adoc[Learn more].
