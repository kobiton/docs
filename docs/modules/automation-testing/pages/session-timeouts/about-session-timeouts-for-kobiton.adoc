...
After link:https://api.kobiton.com/v2/docs#tag/NativeFrameworkAPI/operation/NativeFrameworkAPI_InitiateNativeSession[initiating a session], for xref:capabilities/available-capabilities.adoc#_web_testing[web apps], your initial HTTP request must be sent within 10 minutes, whereas for xref:capabilities/available-capabilities.adoc#_app_testing[native/hybrid apps], the limit is 30 minutes. Every subsequent request, irrespective of the app type, must be sent within 10 minutes.
