= Run an Appium Turbo Test session
...
=== Prerequisites for your automation script

Turbo Test Execution for Appium supports *XIUM* and *Basic Appium 2* using Appium Java client 9.0.0 or later.
...
You will be taken to the API documentation home page. Note down the value of *Base URL (Rest)* as it is the `<api_base_url>` to be used later.

image::PLACEHOLDER_image_7[]

Go back to the Kobiton Portal and note down the value of one of the https://docs.kobiton.com/profile/manage-your-api-credentials#_get_an_api_key[API Keys]. This value, together with the Kobiton account’s username/email, is required to obtain the `<credentials>` to be used later. Add a new key if there aren’t any.
