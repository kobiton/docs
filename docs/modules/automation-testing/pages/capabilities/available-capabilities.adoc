...
[NOTE]
Enabling this capability significantly slows session performance.
...
The passcode type. Use `PIN` or `PASSWORD`.
...
Configure the automation runtime that executes the test. Values are `xium` or `appium`. Default is `xium`.
...
Search every available web context during an Appium find call. For more information, see xref:automation-testing:capabilities/add-web-find.adoc[].
...
Enable xpath2 searches on all supported device platforms. Default is `false`.
