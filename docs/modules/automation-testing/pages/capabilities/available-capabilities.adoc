...
Enables accessibility checks during Appium automation sessions. Set to `true` to capture accessibility validation results and display them in the Session Explorer, matching the behavior of manual sessions. When this capability is omitted from your script, accessibility checks do not run for the session. Accessibility validation is available on paid Kobiton subscriptions.
