...
Filter devices by device tags. Follow 2-dimensional string array formats:
...
* By default, returns matching device(s) with both _Org_ and _Me_ tags. Set `kobiton:deviceTagType` to filter by a single tag type.

====

[IMPORTANT]
See below examples of how to use 2-dimensional string array formats for different programming languages.
