...
Force a reinstall of the app at session start, even if the app is already installed on the device. Use with xref:_fullreset[] to clear app data at session start. For more information, see xref:automation-testing:capabilities/clear-app-state-between-sessions.adoc[window=read-later]. *Only available for private devices.*
...
If the app is already on the device, `fullReset` may not clear app data, because the install step can be skipped. Pair with xref:_enforceappinstall[] to force a reinstall and clear app data. For more information, see xref:automation-testing:capabilities/clear-app-state-between-sessions.adoc[window=read-later].
