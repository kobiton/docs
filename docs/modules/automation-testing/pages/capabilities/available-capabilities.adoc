...
* <<_appInstall,kobiton:appInstall>>
...
Configure application installation when the session is created. Values are `always` or `never`. Default is `always`.
...
[source,java]
----
capabilities.setCapability("kobiton:appInstall", "never");
----
