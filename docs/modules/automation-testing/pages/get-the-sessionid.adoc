= Get the `SessionId`
:navtitle: Get the `SessionId`

Learn how to get the `SessionId` for a completed test session so you can use the link:https://api.kobiton.com/docs/#kobiton-api-v2[Kobiton API] to get session details, collect logs, capture screenshots, and more.
