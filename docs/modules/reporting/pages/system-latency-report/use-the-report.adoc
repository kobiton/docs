...
Learn how to use the system latency report so you can view how long it takes the system to perform an action. See xref:reporting:system-latency-report/report-metadata.adoc[report metadata] to learn more about each field and data type in the report.
...
image:filter-by-connection-context.png[width=1000,alt="A context to the report filtered by connection"]

To filter the report by the physical location of the session, select *By location*.

image:filter-by-location-context.png[width=1000,alt="A context to the report filtered by location"]
