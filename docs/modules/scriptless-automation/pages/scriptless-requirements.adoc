...
* Only 1 application from the Kobiton Application Repository (App Repo) can be launched in the baseline session.

* Applications that is not installed using the App Repo (apps from TestFlight, App Store, Google Play, etc.) need to be pre-installed on the device before the Test Run starts.
