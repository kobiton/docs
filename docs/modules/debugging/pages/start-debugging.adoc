...
[NOTE]
To install apps, use the Kobiton link:https://api.kobiton.com/v2/docs#tag/DeviceAPI/operation/DeviceAPI_InstallDeviceAppVersion[API endpoint,window=read-later] or xref:kobiton-cli:cli-commands.adoc#_app_install[CLI command,window=read-later] instead of a virtualUSB connection. The API and CLI handle larger apps more reliably.
...
* xref:debugging:local-devices/configure-your-personal-computer.adoc[Configure your computer] (Standalone with a self-signed SSL certificate only).
