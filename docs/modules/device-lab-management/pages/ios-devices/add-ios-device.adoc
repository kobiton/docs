...
Follow xref:ios-devices/import-ios-signing-certificates-and-provisioning-profiles.adoc[this section,window=read_later] to import certificates and provisioning profiles to the Mac mini host.

[NOTE]
====

If the iOS/iPadOS device to add is already included in the existing certificates and provisioning profiles on the Mac mini host, skip this step.

To know if the device is included
...
If you see the *deviceControl* warning or the error under the device's name with the message `Device is not associated with any valid installed provisioning profile`, follow this section to troubleshoot the issue and re-generate the certificates and provisioning profiles.
