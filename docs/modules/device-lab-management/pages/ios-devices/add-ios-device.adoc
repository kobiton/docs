...
* Follow this guide to link:prepare-ios-device.adoc[prepare the device].
...
Make sure you have already exported the appropriate link:generate-an-ios-signing-certificate.adoc[developer certificates] and link:generate-an-ios-provisioning-profile.adoc[provisioning profiles] for the UDID of the device.
...
IMPORTANT: Important: Restart deviceConnect services to apply the new provisioning profiles.

=== Import developer certificates and provisioning profiles to deviceShare

TIP: Skip this section if you do not use Kobiton app re-signing service.
...
Make sure you use a supported model of link:../deviceConnect/hardware-requirements-for-deviceconnect.adoc[Cambrionix hub].
