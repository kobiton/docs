...
* _mDNS UDP messages_: Devices broadcast mDNS messages that the Mac mini must receive over UDP port 5353.

* _TCP connection_: Processes on the Mac mini must be able to initiate a TCP connection to the iOS device using one of the IPv6 addresses and port numbers the device advertises. These addresses and port numbers change over time, so the network must allow this dynamic connectivity.

* _Optimal setup_: Place the Mac mini and the iOS devices on the same subnet within the same Wi-Fi network. Most network equipment supports this by default. If your network differs, adjust the router configuration to allow the UDP broadcasts and TCP connections above.

When the network is ready, connect both the devices and the Mac mini host to this network.

[NOTE]
If the network requirements cannot be met, use *Standard mode* to host the iOS 17.0 to 17.3.1 devices instead. You will need at least one additional Mac mini to host in Standard mode.
...
For Lightning mode with iOS 17.0 to 17.3.1, connect the device to one of the USB ports of the Mac mini host to establish trust pairing first. Complete the next section, then connect the device to the Cambrionix hub attached to the GEM.

For Lightning mode with iOS 17.4 and above, connect the device to the Cambrionix hub attached to the GEM.
...
image::device-lab-management:device-lab-management-add-ios-screen-changes-to-blue.PNG[width=300, alt="Device screen changes and shows the Kobiton name and logo"]
...
When the *Trust This Computer* prompt appears on the device, tap *Trust*. If the prompt reappears, tap *Trust* again until there are no more prompts.

image::device-lab-management:device-lab-management-ios-add-ios-standard-lightning-trust.PNG[width=300,alt="Standard mode. Trust This Computer prompt, tapping Trust"]
...
image::device-lab-management:device-lab-management-ios-add-ios-standard-lightning-trust.PNG[width=300,alt="Lightning mode. Trust This Computer prompt, tapping Trust"]

On the Mac mini host, under the *Devices* tab of the *Devices and Simulators* window, the iOS 17 devices appear with a yellow warning message like the one below:
...
The *Trust This Computer* prompt appears again to establish trust pairing with the GEM. Tap *Trust*.

In *Devices and Simulators*, each iOS 17 device now has a globe icon next to its name, as shown below:

image::device-lab-management:device-lab-management-ios-add-ios-xcode-device-and-simulator.PNG[width=300,alt="Devices and Simulators, iPhone is connected notification"]
...
[NOTE]
This section applies only to Mac mini hosts without Internet access that host iOS 17 and above devices.
...
Install *Xcode* on the Internet Mac.
...
Unplug the iOS device from the air-gapped Mac (Standard mode) or the GEM (Lightning mode) and connect it to the Internet Mac.

Open Xcode.

Tap *Trust* in the *Trust This Computer* prompt on the iOS device. If the prompt reappears, tap *Trust* again until there are no more prompts.

In the Xcode menu bar, select *Window → Devices and Simulators*. Select the iOS device under the *Devices* tab.
...
* `/Library/Developer/CoreDevice/CandidateDDIs` (Xcode 16.3 and above). If that folder does not exist, use `~/Library/Developer/CoreDevice/CandidateDDIs`.
...
* `/Library/Developer/CoreDevice/CandidateDDIs` (Xcode 16.3 and above)
...
Continue with connecting iOS devices to the air-gapped Mac mini hosts or the GEM.

[IMPORTANT]
Apple has not published whether the personalized DDI will expire or how long it will last in an air-gapped environment. If connection errors occur and other troubleshooting steps do not resolve the issue, the personalized DDI may have expired, and you will need to repeat this process.
