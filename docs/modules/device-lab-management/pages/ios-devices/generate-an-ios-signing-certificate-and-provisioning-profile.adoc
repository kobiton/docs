...
* It is no longer possible to create App ID with `<Team ID>.*`.

* Create the App ID with `com.\*` as it is always required, then create a new App ID for each custom domain that your apps start with (`org.*`, `net.*`).
...
[IMPORTANT]
As only 1 App ID can be selected per provisioning profile, you must create multiple provisioning profiles if you need to use xref:#_bundle_id[multiple App IDs] for signing.
