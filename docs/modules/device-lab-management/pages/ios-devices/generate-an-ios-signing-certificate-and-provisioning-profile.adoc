...
** If you need to test apps with bundle ID starting with a custom domain, such as `org.company.appname` or `net.company.appname`, *create another App ID* and input the custom domain followed by `.\*`. Example: `org.*`, `net.*`.

[IMPORTANT]

====

* Create the App ID with _com.*_ as it is always required, then create a new App ID for each custom domain that your apps start with (_org.*_, _net.*_).

* Generate a new provisioning profile for each App ID that is created, as you can select only one App ID when creating a profile.
...
[IMPORTANT]
As only one App ID can be selected per provisioning profile, you must create multiple provisioning profiles if you need to use xref:#_bundle_id[multiple App IDs] for signing.
