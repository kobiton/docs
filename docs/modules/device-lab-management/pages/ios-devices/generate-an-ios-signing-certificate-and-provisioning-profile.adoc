...
Add a description. For *Bundle ID*, choose *Wildcard*. Input the appropriate Bundle ID into the box according to the use cases below:
...
image::ios-signing-certificate-register-a-device-closeup.png[width=1000,alt="Plus icon to create a new Profile"]

Select *iOS App Development* under *Development*, then *Continue*.

image::TODO[]

Choose the xref:#_create_an_app_identifier[App ID] you created earlier from the dropdown list, then *Continue*.
...
Select *Download* to save the `.mobileprovision` file for importing to xref:ios-devices/add-ios-device.adoc#_import_provisioning_profiles_to_deviceconnect[deviceConnect] and xref:ios-devices/add-ios-device.adoc#_import_developer_certificates_and_provisioning_profiles_to_deviceshare[deviceShare] later.
