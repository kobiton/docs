...
[IMPORTANT]
If you see the `Failed: An associated private key has restricted access` error under the certificate, set the *Access Control* of its associated private key to  xref:_key_access_control[Allow all applications to access this item], then xref:deviceConnect/restart-deviceconnect-services.adoc[restart deviceConnect,window=read-later] and make sure the error no longer displays before continuing.
...
== Verify certificates and provisioning profiles with device UDIDs

This section provides common ways to verify that the certificates and provisioning profiles matches the connected device's UDID.

[NOTE]
You must follow the guide to xref:device-lab-management:ios-devices/add-ios-device.adoc[add iOS devices,window=read_later] before continuing.

Open the Chrome browser on the Mac mini host, then navigate to the address: *localhost/#/System/IOS*, and log in.

If you have connected the device and deviceConnect can recognize it, it will display in the list under *Devices*.
