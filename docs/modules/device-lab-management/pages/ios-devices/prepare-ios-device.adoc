...
The *Bringing Devices Together* feature, introduced in iOS 17, enables proximity-based interactions between Apple devices.When active, it can interfere with the normal operation of a manual testing session.So, it needs to be turned off when connecting the device to Kobiton.
