...
[#_turn_off_bringing_devices_together_required]
== Turn off Bringing Devices Together (required)

[IMPORTANT]
====

The *Bringing Devices Together* feature, introduced in iOS 17, enables proximity-based interactions between Apple devices. When active, it can interfere with the normal operation of a manual testing session. To prevent issues, make sure to turn it off before connecting the device to Kobiton.
