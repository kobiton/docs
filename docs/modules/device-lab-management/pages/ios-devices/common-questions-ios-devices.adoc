...
== How do I check for error message in Xcode?

Access the Mac mini hosting the device and open Xcode.

On the menu bar, select Windows -> Devices & Simulators

Under the Devices tab, select the device to check.

If the device has any error related to Xcode, the error message displays like below:

[NOTE]
For iOS 17.4 and above hosted in Lightning mode, the devices do not display in the Devices & Simulators window of Xcode. This is normal.
...
=== Standard mode: Need to tap trust on the device

=== iOS 17.0.x to 17.3.x with Lightning mode: Device needs to be trust paired with Mac mini
...
* *Message in the Devices & Simulator window in Xcode*: any of the below:

** The device is not displayed in the list.

** The device is displayed in the list with a message containing: Browsing the local area connection for iPhone/iPad.
...
* *Message in the Devices & Simulator window in Xcode*: Browsing the local area connection for iPhone/iPad.
...
== General errors: Agent not responding, Failed to check passcode state: Connection refused, and A task was canceled.
