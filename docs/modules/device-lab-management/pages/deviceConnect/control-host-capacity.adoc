= Control the capacity of a host machine
:navtitle: Control host machine capacity

== Overview

Configure and manage the maximum number of parallel test sessions that can run on a host machine. This setting acts as a governor to control overall test execution capacity.
...
If a host machine meets Kobiton’s hardware requirements, it can typically support up to 26 parallel sessions.

For this reason, capacity limits are generally required only when more than 26 devices are connected to a single host.

== Set the Max Parallel Tests

Capacity limits for every host machine are configured from one dialog in Device Management.
...
. Open the *Max Parallel Tests* tab.

. Find the host machine you want to configure. Search by machine name or IP address.

. In the machine's row, choose one of the following options:

* *Unlimited* (default): no limit for the host machine.

* *Custom limit*: set a limit between 1 and 999. Enter the value in the input next to the option.
+
The recommended value is *26*.

. To apply the same setting to every host machine at once, use the *All Machines* row at the top of the list. Changing an individual machine afterward sets the *All Machines* row to *Mixed*.

. Select *Save*.

A machine that is offline when you save is not updated. The dialog stays open and reports which machines were rejected.

// REVIEWER NOTE: screenshots for this procedure were removed because they show the previous
// per-machine Settings button and per-machine pop-up, which no longer exist. Replace with
// captures of the Settings dialog on the Max Parallel Tests tab. The unused image files
// host-machine-capacity-settings.png and host-machine-set-capacity.png are left in place
// pending that replacement.

// REVIEWER NOTE: the custom-limit option label is inconsistent across the source evidence
// ("Set custom limit" in the previous per-machine modal, "Custom limit" in the dialog
// specification, "Custom" in the design mockups). This draft uses "Custom limit".
// Confirm the shipped label.
...
When the capacity limit is reached, devices hosted on the machine remain *online* but are not immediately available for launch. By default, these devices are hidden in the *Devices* list.
