...
To check whether an update is available, open the *Device Management* tab in *Settings* and review the *deviceConnect* version listed for each Mac mini host.
...
* Devices are not taken offline because of the *deviceConnect* version running on their host. A host running an older version keeps its devices available for sessions.
* To stay within the supported range, leave automatic updates enabled, or update manually from *Device Management*.