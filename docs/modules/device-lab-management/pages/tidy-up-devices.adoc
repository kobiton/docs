...
+
A `-` indicates that the device was offline before the Tidy Up feature was introduced and is safe to remove.

+
Review devices with low offline counts
...
Selected devices are hidden and no longer appear in the device list. If a hidden device is plugged back in, it will become online again.