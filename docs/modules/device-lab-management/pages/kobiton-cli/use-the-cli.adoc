...
== Before the user start

* Obtain the username and API key from a Kobiton Enterprise account.
* Ensure private devices are available in the Kobiton Portal.
...
If the user ran into an error while running the CLI commands, try the below steps:
...
|`The credential the user entered was not authorized. Please double-check and try again.`
