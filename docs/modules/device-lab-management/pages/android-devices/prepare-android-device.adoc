...
Locate the operating system version field and tap 7 times on it. This field is named **MIUI Version** on MIUI and **OS version** on HyperOS.
...
In **Developer options**, turn off the system optimization setting. Its name depends on the version of the operating system:

* On **MIUI**, disable **MIUI Optimization**.

* On **HyperOS**, disable **Turn on system optimization**. Select **Accept** to confirm.

If you cannot find the option, tap the **Reset to default values** field several times to reveal it: 4 times on MIUI, 6 times on HyperOS.

[IMPORTANT]
Leaving system optimization on prevents Kobiton software from installing and updating the device control app on the device. A permission popup displays during installation, and the device may not come back online after a restart.

image:device-lab-management:device-lab-management-android-chinese-developer-options.png[width=240, alt="Enabling Install via USB and USB Debuigging (security setting) inside Developer Options"]  -> image:device-lab-management:device-lab-management-android-chinese-developer-options-reset-to-default-value.png[width=240, alt="In Developer options, tapping Reset to default values field 4 times"] -> image:device-lab-management:device-lab-management-android-chinese-developer-options-disable-miui-optimization.png[width=240, alt="Turning off MIUI iptimization in Developer options"]
