...
Tap 7 times on **MIUI Version**, or **OS version** on HyperOS.

image:device-lab-management:device-lab-management-android-chinese-about-phone.png[width=240, alt="Going to About Phone in Settings"] -> image:device-lab-management:device-lab-management-android-chinese-miui.png[width=240, alt="Tapping the operating system version field several times"]

The message **You're now a developer!** displays. The Developer mode is now activated.

Find and open **Developer options** in **Settings**. On HyperOS, this option is under **Additional settings**.

Enable **Install via USB** and **USB Debugging (security setting)**.
...
In **Developer options**, turn off system optimization.

On many models this setting is hidden. If you do not see it, tap the **Reset to default values** field repeatedly until it appears. The number of taps varies by model, and is usually 4.

Disable **MIUI Optimization** on MIUI, or **Turn on system optimization** on HyperOS. Select **Accept** to confirm.

[IMPORTANT]
Leaving system optimization on prevents Kobiton software from installing and updating deviceControl on the device. A permission pop-up displays during installation, and the device does not come back online after a restart.

image:device-lab-management:device-lab-management-android-chinese-developer-options.png[width=240, alt="Enabling Install via USB and USB Debugging (security setting) inside Developer options"]  -> image:device-lab-management:device-lab-management-android-chinese-developer-options-reset-to-default-value.png[width=240, alt="In Developer options, tapping the Reset to default values field several times"] -> image:device-lab-management:device-lab-management-android-chinese-developer-options-disable-miui-optimization.png[width=240, alt="Turning off system optimization in Developer options"]
...
[#disable_permission_monitoring]
== Turn on _Disable permission monitoring_ (required for automation on some models)

[IMPORTANT]
====

On some Android models, permission monitoring revokes permissions that Kobiton software grants itself during a session. Automation sessions on these devices fail to start, or stop responding partway through.
...
Scroll down to the **Disable permission monitoring** option and turn it on. You can use the search function to quickly locate it.

[NOTE]
The option name describes what it turns off. Turning the option _on_ disables permission monitoring, which is what Kobiton requires.
...
== Turn off _Auto Blocker_ and _Block USB connections while locked_ (Samsung devices with Android 14 and later)

[IMPORTANT]
====

On Samsung devices running One UI 6.0 (Android 14) and later:

* The *Block USB connections while locked* feature will disconnect the device's USB connection after the lock screen is active for more than 1 hour.

* The *Auto Blocker* feature will prevent Kobiton from controlling the device.

Both features can cause devices to go offline.

====

To turn these features off, unlock the device if it is locked.

Navigate to *Settings* -> *Security and privacy*.

Turn off *Auto Blocker*.

image:device-lab-management:android-prepare-security-privacy-details.png[width=240, alt="The Auto Blocker security feature is disabled in the Security and Privacy settings on Android."]

Select *More security settings*, then turn off *Block USB connections while locked*.

image:device-lab-management:android-prepare-more-security-settings.png[width=240, alt="The Block USB connections while locked setting is turned off in the More Security Settings screen on Android."]
...
Google Play Protect will attempt to scan the Kobiton agent app and may prevent it from controlling the device.
...
image:device-lab-management-android-prepare-play-protect-settings.png[width=240, alt="The Play Protect settings screen with the options Scan apps with Play Protect and Improve harmful app detection turned off"]
