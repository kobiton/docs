...
[source,]
----
down(0%, 0%, 3) wait(10) move(50%, 50%, 3) wait(10) up(3) // Places the 4th finger at 0%, 0%, moves it to 50%, 50%, then removes it. wait() lets each action complete before the next.
----
...
== Common gesture recipes

These recipes combine xref:_functions[functions] into complete gestures. The `wait()` durations shown are starting points. Adjust them based on the app's behavior and the device's response.

[#_two_finger_tap]
=== Two-finger tap

Two adjacent fingers tap the screen three times.
...
A single finger taps the center of the screen twice.
...
[#_single_finger_swipe]
=== Single-finger swipe

One finger swipes from left to right across the screen.
...
Two fingers swipe upward in parallel.
