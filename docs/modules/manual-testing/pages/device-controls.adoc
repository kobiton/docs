...
image:select-touch-context.png[width=1000,alt="Select Touch on Manual session"]
...
image:volume-up-volume-down-context.png[width=1000,alt="Select Volume up or Volume Down"]

[#_adb_shell]
== `adb` shell

To enter Android Debug Bridge (`adb`) commands, select *adb shell*, then enter a xref:adb-commands.adoc[supported `adb` command].
...
image:perform-voiceover-context.png[width=1000,alt="Select Perform Voiceover feature"]

== Biometric authentication

Once our xref:integrations:biometric-authentication/about-biometrics-authentication.adoc[biometrics authentication library] is added to your app, select *Biometric Authentication*, then choose *Pass* or *Fail* to test authentication requests.

image:biometrics-authentication-context.png[width=1000,alt="Select Biometric Authentication feature"]

[#_draw_custom_gesture]
== Draw custom gesture

To draw custom gestures like three-finger swipes or two-finger taps, select *Draw Custom Gesture,* then choose a sample gesture or xref:custom-gestures.adoc[create your own].
...
image:save-custom-gesture-closeup.png[width=300,alt="Select the star icon to save the custom gesture"]

== Input sensitive data

To input sensitive data (like usernames and passcodes) without it being recorded in the xref:session-analytics:session-overview.adoc#_session_recording[session video], select *Input Sensitive Data*, then enter the data in the text box.
...
image:save-input-sensitive-data-closeup.png[width=300,alt="Select Save to input the sensitive data"]

== Image injection

To xref:test-the-app-camera.adoc[test your in-app camera], select *Image Injection*, then add an image to the injection library and select *Inject*.
...
image:power-button-context.png[width=1000,alt="Select Power button"]
...
image:home-button-context.png[width=1000,alt="Select Home button"]
...
image:capture-context.png[width=1000,alt="Enable Synchronous Inventory Capture"]
...
image:select-screenshot-closeup.png[width=750,alt="Select screenshots you want to download"]
