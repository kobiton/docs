...
These are the device controls you can use to interact with a device during a manual session.
...
After you set up xref:devices:local-devices/configure-ios-voiceover.adoc[VoiceOver for iOS], you can test your app's screen reader compatibility.

== Biometric authentication

After you add our xref:integrations:biometric-authentication/about-biometrics-authentication.adoc[biometrics authentication library] to your app, you can select *Pass* or *Fail* to test biometric authentication requests.
...
You can input sensitive data (like usernames and passcodes) without recording it in the xref:session-analytics:session-overview.adoc#_session_recording[session video]. Enter your sensitive data, tap the related field on your device, then select *Save* to send it securely.
...
You can xref:test-the-app-camera.adoc[test your in-app camera] by adding an image to the injection library and selecting *Inject*.
...
You can single-click or double-click the *Home Button* to replicate the link:https://support.apple.com/HT208204[iOS] or link:https://support.google.com/android/answer/9079644[Android] home button.
