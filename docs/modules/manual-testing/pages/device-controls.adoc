...
To input sensitive data (like usernames and passwords) without it being recorded in the xref:session-analytics:session-overview.adoc#_session_recording[session video], select *Input Sensitive Data*, then enter the data in the text box.
...
To xref:manual-testing:test-the-app-camera.adoc[test your in-app camera], select *Image Injection*, then add an image to the injection library and select *Inject*.
