...
Once xref:devices:local-devices/configure-ios-voiceover.adoc[VoiceOver for iOS] is set up, select *Preform VoiceOver* to test an app's screen reader compatibility.
...
To input sensitive data (like usernames and passcodes) without it being recorded in the xref:session-explorer:manage-a-session.adoc#_watch_the_recording[session video], select *Input Sensitive Data*, then enter the data in the text box.
