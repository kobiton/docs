...
Use VoiceOver on iOS devices or TalkBack on Android devices to test app accessibility. +
...
For more information, see the xref:screen-reader/voice-over-manual-session.adoc[VoiceOver guide,window=read-later] and xref:screen-reader/talkback-manual-session.adoc[TalkBack guide,window=read-later].
