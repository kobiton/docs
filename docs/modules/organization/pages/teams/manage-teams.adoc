= Manage teams
:navtitle: Manage teams

Learn how to manage your team so you can clone your team, add team members, set member permissions, and more. xref:organization:teams/search-for-a-team.adoc[Search for a team] to get started.

== Clone the team

In *Details*, select *Clone* to create a copy of the team including all settings, users, permissions, and devices.

image:organization:clone-a-team-closeup.png[width=1000,alt="A closeup to clone a team function"]

When you're finished, select *Save*.

image:organization:save-cloned-team-context.png[width=1000,alt="Save cloned team"]

== Disable the team

In *Details*, select *Disable* to disable the team.

image:organization:disable-a-team-context.png[width=1000,alt="click Disable to disable a team"]

The disabled team appears grayed out like the below:

image:inactived-team-closeup.png[width=1000,alt="A closeup to a disabled team"]

== Make the team your default

In *Details*, choose the team to be your xref:profile:manage-your-profile.adoc#_change_your_default_team[default team].

image:organization:mark-as-default-team-context.png[width=1000,alt="Mark a team as the default Team"]

When you're finished, select *Save*.

image:organization:mark-as-default-pop-up-closeup.png[width=500,alt="Mark as Default popup"]

== Delete the team

In *Details*, select *Delete* to delete the team.

[NOTE]
Deleting a team cannot be undone.

image:organization:delete-a-team-closeup.png[width=1000,alt="A closeup to Delete button to remove a team"]

When you're finished, select *Save*.

image:organization:delete-popup-closeup.png[width=500,alt="A closeup to Delete Team popup"]

[#_private_test_mode]
== Enable private test mode

In *Settings*, select *Enable Private Test Mode* to turn off session recordings for all private devices, which includes any commands, screenshots, or videos typically recorded in a session.

image:organization:private-test-mode-closeup.png[width=1000,alt="A closeup to Private Test Mode"]

When you're finished, select *Save*.

image:organization:private-test-mode-context.png[width=1000,alt="A context to Private Test Mode"]

[#_cleanup_policy]
== Set a cleanup policy

In *Settings*, select the dropdown and choose a default cleanup policy, or xref:organization:teams/create-a-device-cleanup-policy.adoc[create a new one] for the team. Team devices will be cleaned up after every session, so they're ready to use in the next one.

image:organization:cleanup-policies-closeup.png[width=1000,alt="A closeup to Cleanup policies"]

When you're finished, select *Save*.

image:organization:save-cleanup-policies-context.png[width=1000,alt="A context to save newly changed Cleanup policy"]

[#_add_a_new_user]
== Add a new user

In *Users*, select *Add new user*, then choose a role: *Team Owner*, *Team Admin*, or *Team Member*.

image:organization:add-user-team-role-closeup.png[width=500,alt="Choose a Team Role"]

After you choose a role, search for a user.

[NOTE]
A user must be xref:organization:users/invite-a-user.adoc[invited to your organization] before you can add them to a team.

image:organization:search-users-team-role.png[width=500,alt="A closeup to search for a user in the Add New Users modal"]

Choose the user, then select *Update*.

When you're finished, select *Save*.

image:organization:click-save-after-add-user-context.png[width=1000,alt="click Save after adding a user to a Team Role"]

== Remove a user

In *Users*, search for a user, then select the *minus* icon.

image:organization:remove-users-from-team-context.png[width=1000,alt="Remove a user from a Team"]

When you're finished, select *Save*.

image:organization:click-save-remove-users-context.png[width=1000,alt="Click Save after removing a user"]

[#_set_team_permissions]
== Set team permissions

In *Permissions*, choose permissions for each xref:organization:teams/team-roles-and-permissions.adoc[team role].

image:organization:team-role-permissions-context.png[width=1000,alt="A context to team role permissions"]

When you're finished, select *Save*.

== Manage devices

In *Device*, you can xref:organization:teams/manage-team-devices.adoc[manage your team's devices].

image:organization:devices-tab-context.png[width=1000,alt="A context to Devices tab"]
