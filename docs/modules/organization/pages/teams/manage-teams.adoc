...
[#_cleanup_policy]
=== Set a default cleanup policy

In *Settings*, select the dropdown and choose a default cleanup policy or xref:devices:local-devices/cleanup-policy/manage-device-cleanup-policies.adoc#_create_a_cleanup_policy[create a new one,window=read-later]. Team devices are cleaned up after every session, so they're ready to use for the next one.
