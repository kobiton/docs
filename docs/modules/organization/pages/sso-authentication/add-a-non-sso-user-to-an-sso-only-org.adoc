...
* Any API request to link:https://api.kobiton.com/v2/docs#tag/OrganizationAPI/operation/OrganizationAPI_CreateOrgMember[create new member,window=read-later] returns response `400` with the message _Cannot invite new user because the organization enforces its users to login/signup to Kobiton only through SSO_
