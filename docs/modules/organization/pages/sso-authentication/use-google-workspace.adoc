...
In  *App details*, enter a name for your application.
...
In **Google Identity Provider Details**, copy and save the following IdP values.

[cols=“1,1”]
|===
| Kobiton field | Google value
...
In Service Provider Details, use your Kobiton SSO parameters from earlier to fill out the Google service provider fields.

[cols=“1,1”]
|===
| Google field | Kobiton value
...
Fill in the ACS URL, Entity ID, and Domain Key fields, then continue to the next step.

image:organization:gsuite-fill-in-acs-id-domain-context.png[width=1000,alt="Fill in ACS URL, Entity ID, Domain Key"]

In **Add custom SAML app**, add the following attributes.
...
include::partial$sso-authentication/sso-provider-url-mappings.adoc[tag=google]
