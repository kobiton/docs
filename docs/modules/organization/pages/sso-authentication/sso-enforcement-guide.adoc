...
When *Enforce SSO* is enabled, the *Choose users who are allowed to login without SSO* field appears. Add users to this list to exempt them from organization-wide SSO enforcement. For steps, see xref:organization:sso-authentication/add-a-non-sso-user-to-an-sso-only-org.adoc[].
