...
When role and team pass-through is turned off, the *SSO Attribute Value* field is hidden.

== Role and team synchronization rules

When role and team pass-through is enabled, Kobiton synchronizes roles and team assignments from IdP group memberships during SSO login.
