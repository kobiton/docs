...
NOTE: Learn more about xref:organization:sso-authentication/sso-enforcement-guide.adoc[window=read-later] and xref:organization:sso-authentication/pass-role-and-team-assignments.adoc[window=read-later].